 ...
 */
package org.geosde.compute.process.function;
...
import org.geosde.compute.process.ProcessFactory;
import org.geosde.compute.process.Processors;
import org.geosde.compute.process.RenderingProcess;
...
/**
 * A bridge between the process world and the filter function world: any process
 * returning a single value can be seen as a filter function
 * 
...
public class ProcessFunctionFactory implements FunctionFactory {
	...
	/**
	 * Parameter used to indicate a certain output is the primary one for the
	 * process
	 */
	...
	public Function function(Name processName, List<Expression> args, Literal fallback) {
		...
		if (!processToFunction.containsKey(processName)) {
			...
			// wrap the process
			org.geosde.compute.process.Process process = Processors.createProcess(processName);
			Map<String, Parameter<?>> parameters = Processors.getParameterInfo(processName);
			if (process instanceof RenderingProcess) {
				return new RenderingProcessFunction(processName, args, parameters, (RenderingProcess) process,
						fallback);
			} else {
	...
	private synchronized void init() {
		if (functionNames == null) {
			// collect and sort the factories to have a reproducable list of
			// function names
			...
			// collect name and params of all processes resulting in a single
			// output
				...
				for (Name processName : factory.getNames()) {
					try {
						...
						if (result != null) {
							...
							List<org.opengis.parameter.Parameter<?>> args = new ArrayList<org.opengis.parameter.Parameter<?>>(
									argumentNames.size());
